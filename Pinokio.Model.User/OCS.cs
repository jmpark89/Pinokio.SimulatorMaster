        ...
        private List<TransportLine> CalculateRouteToCurrent(TransportLine endLine, TransportLine curLine, string bayName)
        {
            ...
            while (tempLine != curLine)
            {
                foreach (TransportLine inputLine in tempLine.StartPoint.InLines)
                {
                    ...
                    if (tempLine.StartPoint.InLines.Count > 1 && tempLine.StartPoint.InLines.All(x => ((OHTLine)x).Bay == bayName))
                    {
                        double minLen = inputLine.Length;
                        double tempLen = tempLine.StartPoint.InLines[1].Length;
                        if (minLen < tempLen)
                        {
                        ...
                        {
                            tempRoute.Add(tempLine.StartPoint.InLines[1]);
                            tempLine = tempLine.StartPoint.InLines[1];
                        }
                        break;
                    }
