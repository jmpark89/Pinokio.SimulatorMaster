...
namespace Pinokio.Model.User
{
    public class LCS : VSubCS
    {
        ...
        [Browsable(false)]
        public Dictionary<string, ElevatorLine> DicRailLine
        {
            get
            {
                return _dicRailLine;
        ...
        public void Initialize()
        {
            ...
            _dicRailLine = new Dictionary<string, ElevatorLine>();
        }
        ...
        public override void InitializeNode(EventCalendar evtCal)
        {
            ...
            foreach (Elevator elevator in Vehicles)
            {
                SetIdleDestinationAndRoute(0, elevator);
            }
        ...
        public override void AddChildNode(SimNode node)
        {
            ...
            else if (node is ElevatorLine && DicRailLine.ContainsKey(node.Name) is false)
                DicRailLine.Add(node.Name, node as ElevatorLine);
        ...
        public override void ArriveAtCoupled(Time simTime, SimPort port, SimNode greetNode)
        {
            ...
            cmd.StartStation = ((ElevatorLine)greetNode).InLinkNodeConnections[port.FromNode as TXNode];
            int endNodeIndex = part.MR.Route.IndexOf(cmd.EndNode);
            cmd.EndStation = ((ElevatorLine)part.MR.Route[endNodeIndex - 1]).OutLinkNodeConnections[cmd.EndNode];
        ...
        public override void SetIdleDestinationAndRoute(Time simTime, Vehicle vehicle)
        {
            try
            {
                Elevator elevator = (Elevator)vehicle;

                elevator.Destination = LineStations.First();
            }
            catch (Exception ex)
            {
                throw;
