...
{
    [Serializable]
    public class TCS : VSubCS
    {
        private Dictionary<string, TrainLine> _dicRailLine;
        private Dictionary<string, TrainPoint> _dicRailPoint;
//        private Dictionary<string, TrainZCU> _zcus;
        private Dictionary<string, BAY> _bays;

        [Browsable(true), StorableAttribute(true)]
        [CategoryAttribute("5. TCS Info"), DisplayName("1. Bump Distance")]
        public double BumpDistance { get; set; }

        [Browsable(true), StorableAttribute(true)]
        [CategoryAttribute("5. TCS Info"), DisplayName("2. Dispatching Interval Time")]
        public double DispatchingIntervalTime { get; set; }

        [Browsable(false)]
        public Dictionary<string, TrainLine> DicRailLine
        {
            get
            {
                return _dicRailLine;
        ...
        [Browsable(false)]
        public Dictionary<string, TrainPoint> DicRailPoint
        {
            get
            {
        ...
        //[Browsable(false)]
        //public Dictionary<string, TrainZCU> Zcus
        //{
        //    get { return _zcus; }
        //}
        ...
        public void Initialize()
        {
            ...
            _dicRailPoint = new Dictionary<string, TrainPoint>();
            _dicRailLine = new Dictionary<string, TrainLine>();
//            _zcus = new Dictionary<string, TrainZCU>();
        ...
        {
            InitializeLineNetworksByAngle();
            base.InitializeNode(evtCal);

            foreach (Vehicle train in Vehicles)
            {
                SetIdleDestinationAndRoute(0, train);
            }
        ...
        private void InitializeLineNetworksByAngle()
        {
            foreach(TrainLine line in Nodes.Where(x => x is TrainLine))
            {
                line.InitializeLineNetworkByAngle();
            }
        }
        ...
        //public TrainZCU AddZcu(string name)
        //{
        //    TrainZCU zcu = new TrainZCU(name, this);
        ...
        public override void AddChildNode(SimNode node)
        {
            ...
            else if (node is TrainLine && DicRailLine.ContainsKey(node.Name) is false)
                DicRailLine.Add(node.Name, node as TrainLine);
            else if (node is TrainPoint && DicRailPoint.ContainsKey(node.Name) is false)
            {
                DicRailPoint.Add(node.Name, node as TrainPoint);
                //if (((TrainPoint)node).Zcu != null && Zcus.ContainsKey(((TrainPoint)node).Zcu.Name) is false)
                //    Zcus.Add(((TrainPoint)node).Zcu.Name, ((TrainPoint)node).Zcu);
            }
        }
        ...
        public override void ArriveAtCoupled(Time simTime, SimPort port, SimNode greetNode)
        {
            ...
            cmd.StartStation = ((TrainLine)greetNode).InLinkNodeConnections[port.FromNode as TXNode];
            int endNodeIndex = part.MR.Route.IndexOf(cmd.EndNode);
            cmd.EndStation = ((TrainLine)part.MR.Route[endNodeIndex - 1]).OutLinkNodeConnections[cmd.EndNode];
